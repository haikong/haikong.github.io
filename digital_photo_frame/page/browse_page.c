 ...
 ******************************************************************************
  ...
  生成日期   : 2014年11月4日
  最近修改   :
  功能描述   : 浏览模式，显示浏览的文件夹和文件
  函数列表   :
              BrowsePageGetInputEvent
              BrowsePageInit
              BrowsePagePrepare
              BrowsePageRun
              CalcBrowsePageDirAndFilesLayout
              CalcBrowsePageLayout
              DeSelectDirFileIcon
              GenerateBrowsePageDirAndFile
              GenerateDirAndFileIcons
              GetInputPositionInPageLayout
              GetSelectedDir
              SelectDirFileIcon
              ShowBrowsePage
  修改历史   :
...
static int g_iDirFileNumPerCol, g_iDirFileNumPerRow;//一行一列目录文件数量
...
static int GenerateDirAndFileIcons(PT_PageLayout ptPageLayout)
{
    ...
    g_tDirClosedIconPixelDatas.iWith= atLayout[0].iBotRightX - atLayout[0].iTopLeftX + 1;
    g_tDirClosedIconPixelDatas.iLineByteCnt= g_tDirClosedIconPixelDatas.iWith * g_tDirClosedIconPixelDatas.iBpp / 8;
    g_tDirClosedIconPixelDatas.iTotalBytes = g_tDirClosedIconPixelDatas.iLineByteCnt * g_tDirClosedIconPixelDatas.iHeight;
    ...
    g_tDirOpenedIconPixelDatas.iWith  = atLayout[0].iBotRightX - atLayout[0].iTopLeftX + 1;
    g_tDirOpenedIconPixelDatas.iLineByteCnt  = g_tDirOpenedIconPixelDatas.iWith * g_tDirOpenedIconPixelDatas.iBpp / 8;
    g_tDirOpenedIconPixelDatas.iTotalBytes = g_tDirOpenedIconPixelDatas.iLineByteCnt * g_tDirOpenedIconPixelDatas.iHeight;
    ...
    g_tFileIconPixelDatas.iWith  = atLayout[0].iBotRightX - atLayout[0].iTopLeftX+ 1;
    g_tFileIconPixelDatas.iLineByteCnt  = g_tDirClosedIconPixelDatas.iWith * g_tDirClosedIconPixelDatas.iBpp / 8;
    g_tFileIconPixelDatas.iTotalBytes = g_tFileIconPixelDatas.iLineByteCnt * g_tFileIconPixelDatas.iHeight;
...
static void ShowBrowsePage(PT_PageLayout ptPageLayout)
{
	...
	/* 1. 获得显存 */
	ptVideoMem = GetVedioMem(ID("browse"), 1);
	if (ptVideoMem == NULL)
	{
...
static int BrowsePagePrepare(void)
{
    return 0;
}
