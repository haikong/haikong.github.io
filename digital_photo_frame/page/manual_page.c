...
static PT_PixelDatas GetZoomedPicPixelDatas(PT_PixelDatas ptOriginPicPixelDatas, int iZoomedWidth, int iZoomedHeight)
{
    ...
    k = (float)ptOriginPicPixelDatas->iHeight / ptOriginPicPixelDatas->iWith;
    g_tZoomedPicPixelDatas.iWith  = iZoomedWidth;
    g_tZoomedPicPixelDatas.iHeight = iZoomedWidth * k;
    if (g_tZoomedPicPixelDatas.iHeight > iZoomedHeight)
    {
        g_tZoomedPicPixelDatas.iWith  = iZoomedHeight / k;
        g_tZoomedPicPixelDatas.iHeight = iZoomedHeight;
    }
    g_tZoomedPicPixelDatas.iBpp        = iBpp;
    g_tZoomedPicPixelDatas.iLineByteCnt= g_tZoomedPicPixelDatas.iWith * g_tZoomedPicPixelDatas.iBpp / 8;
    g_tZoomedPicPixelDatas.iTotalBytes = g_tZoomedPicPixelDatas.iLineByteCnt* g_tZoomedPicPixelDatas.iHeight;
...
static int ShowPictureInManualPage(PT_VideoMem ptVideoMem, char *strFileName)
{
    ...
    /* 算出居中显示时左上角坐标 */
    iTopLeftX = g_tManualPictureLayout.iTopLeftX + (iPictureLayoutWidth - ptZoomedPicPixelDatas->iWith) / 2;
    iTopLeftY = g_tManualPictureLayout.iTopLeftY + (iPictureLayoutHeight - ptZoomedPicPixelDatas->iHeight) / 2;
    g_iXofZoomedPicShowInCenter = ptZoomedPicPixelDatas->iWith / 2;
...
static void ShowManualPage(PT_PageLayout ptPageLayout, char *strFileName)
{
	...
	/* 1. 获得显存 */
	ptVideoMem = GetVedioMem(ID("manual"), 1);
	if (ptVideoMem == NULL)
	{
...
static void ShowZoomedPictureInLayout(PT_PixelDatas ptZoomedPicPixelDatas, PT_VideoMem ptVideoMem)
{
    ...
    if (iStartXofNewPic > ptZoomedPicPixelDatas->iWith)
    {
        iStartXofNewPic = ptZoomedPicPixelDatas->iWith;
    }
    ...
    if ((ptZoomedPicPixelDatas->iWith- iStartXofNewPic) > (g_tManualPictureLayout.iBotRightX - iStartXofOldPic + 1))
        iWidthPictureInPlay = (g_tManualPictureLayout.iBotRightX - iStartXofOldPic + 1);
    else
        iWidthPictureInPlay = (ptZoomedPicPixelDatas->iWith - iStartXofNewPic);
                ...
                if (bButtonPressed)
                {
                    ...
                    if (iIndexPressed == iIndex) /* 按下和松开都是同一个按钮 */
                    {
                        switch (iIndexPressed)
                        {
                            ...
                            {
                                /* 获得缩小后的数据 */
                                iZoomedWidth  = (float)g_tZoomedPicPixelDatas.iWith* ZOOM_RATIO;
