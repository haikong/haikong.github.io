...
static int g_iIntervalSecond = 1;   /*显示数字默认值*/
...
static void  IntervalPageCalcLayout(PT_PageLayout ptPageLayout)
{
	...
	iError = GetDispResolution(&iXres, &iYres, &iBpp);
    if(iError){
        DBG_PRINTF("%s : %d GetDispResolution failed.\n",__FUNCTION__,__LINE__);
        return ;
    }
	...
	DBG_PRINTF("frame:  iWidth = %d, iHeight = %d\n", iWidth, iHeight);
	DBG_PRINTF("number: iWidth = %d, iHeight = %d\n", g_tIntervalNumberLayout.iBotRightX - g_tIntervalNumberLayout.iTopLeftX, g_tIntervalNumberLayout.iBotRightY - g_tIntervalNumberLayout.iTopLeftY);
...
static void ShowIntervalPage(PT_PageLayout ptPageLayout)
{
	...
	/* 1. 获得显存 */
	ptVideoMem = GetVedioMem(ID("interval"), 1);
	if (ptVideoMem == NULL)
	{
...
static void IntervalPageRun(PT_PageParams ptParentPageParams)
{
	...
	/* 这两句只是为了避免编译警告 */
    tInputEvent.tTime.tv_sec = 0;
    tInputEvent.tTime.tv_usec = 0;
		...
		else
		{
			...
			if (iIndex != -1)
			{
				...
				if ((iIndexPressed == 0) || (iIndexPressed == 2))
				{
					...
					{
						iNumber = iIndexPressed ? (iNumber - 1) : (iNumber + 1);
						if (iNumber == 60)
						{
							iNumber = 0;
						}
