...
static PT_PageAction g_ptPageAction;
...
int RegisterPageAction(PT_PageAction ptPageAction)
{
	...
	if (!g_ptPageAction)
	{
		g_ptPageAction   = ptPageAction;
		ptPageAction->ptNext = NULL;
	}
	else
	{
		ptTmp = g_ptPageAction;
		while (ptTmp->ptNext)
		{
			ptTmp = ptTmp->ptNext;
		}
...
{
	int i = 0;
	PT_PageAction ptTmp = g_ptPageAction;
...
int GeneratePage(PT_PageLayout ptPageLayout, PT_VideoMem ptVideoMem)
{
	...
	if (ptVideoMem->ePicState != PS_GENERATED)
	{
		...
		while (atLayout->strIconName)
		{
            ...
            DBG_PRINTF("iconname =%s\n", atLayout->strIconName);
 			tIconPixelDatas.iHeight = atLayout->iBotRightY - atLayout->iTopLeftY + 1;
			tIconPixelDatas.iWith  = atLayout->iBotRightX - atLayout->iTopLeftX+ 1;
			tIconPixelDatas.iLineByteCnt= tIconPixelDatas.iWith * tIconPixelDatas.iBpp / 8;
			tIconPixelDatas.iTotalBytes = tIconPixelDatas.iLineByteCnt * tIconPixelDatas.iHeight;
