...
static PT_VideoMem PrepareNextPicture(int bCur)
{
    ...
    k = (float)tOriginIconPixelDatas.iHeight / tOriginIconPixelDatas.iWith;
    tPicPixelDatas.iWith= iXres;
    tPicPixelDatas.iHeight = iXres * k;
    if (tPicPixelDatas.iHeight > iYres)
    {
        tPicPixelDatas.iWith  = iYres / k;
        tPicPixelDatas.iHeight = iYres;
    }
    tPicPixelDatas.iBpp        = iBpp;
    tPicPixelDatas.iLineByteCnt= tPicPixelDatas.iWith * tPicPixelDatas.iBpp / 8;
    tPicPixelDatas.iTotalBytes = tPicPixelDatas.iLineByteCnt * tPicPixelDatas.iHeight;
    ...
    /* 算出居中显示时左上角坐标 */
    iTopLeftX = (iXres - tPicPixelDatas.iWith) / 2;
