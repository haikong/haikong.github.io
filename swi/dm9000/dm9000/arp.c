...
unsigned short ARP_pack(unsigned char *datas,unsigned short length,e_MAC_PROTO proto)
{
	...
	ARPBUF->hwtype = htons( 1 ); //硬件类型(1 表示传输的是以太网 MAC 地址)
	ARPBUF->protocol = htons( 0x0800 ); //协议类型(0x0800 表示传输的是 IP 地址)
	ARPBUF->hwlen = 6;
	ARPBUF->protolen = 4;
...
unsigned char UDP_pack(unsigned char *datas,unsigned short length)
{
	...
	UDPBUF->sport=htons(1024); //源端口号设置 1024=0x0400,直接写 1024 的话，小端模式中第一个字节为 0，第二个字节为 4，
	//但是接收方会认为先收到的为高位，结果成了 0x0004,所以应该先互换高低位写进去

	UDPBUF->dport=htons(2844); //目的端口号设置
	UDPBUF->length=htons(length+8); //UDP 包长度=数据部分长度+UDP 报头长度
	UDPBUF->udpchksum=0; //UDP 校验和初始化为


	UDPBUF->udpchksum=htons(udpchecksum((unsigned char*)packhead,datas,length)); //UDP 检验和
