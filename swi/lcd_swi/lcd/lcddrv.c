...
static unsigned int g_fb_base_addr;      //frambuffer base address
static unsigned int g_bpp;               //lcd pixel
static unsigned int g_xsize;             //lcd x-size
static unsigned int g_ysize;             //lcd y-size
...
void Tft_Lcd_Init(int type)
{
    ...
    case MODE_TFT_16BIT_640480:
		...
		case MODE_TFT_24BIT_480272:
			...
			LCDCON2 = (VBPD_272480<<24) | (LINEVAL_TFT_272480<<14) | \
			 ...
			 *	  设置LCDBANK、LCDBASEU
			 * 2. LCDSADDR2:
			 *	  设置LCDBASEL: 帧缓冲区的结束地址A[21:1],考虑每个像素大小
			 * 3. LCDSADDR3:
			 *	  OFFSIZE等于0，PAGEWIDTH等于(480*4/2)
			 */
			LCDSADDR1 = ((LCDFRAMEBUFFER>>22)<<21) | LOWER21BITS(LCDFRAMEBUFFER>>1);
			LCDSADDR2 = LOWER21BITS((LCDFRAMEBUFFER+ \
