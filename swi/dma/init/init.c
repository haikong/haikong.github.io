...
*attention: this function not used as nand init
*/
...
void timer_init(void)
{
	...
	TCFG1 	= 0x00;		//选择PWM定时器0的选通输入,1/2 
	TCNTB0 	= 50000;		//1s 
	TCMPB0 	= 500;		//0.5s pwm = 1/3
	TCON  	= (1 << 1);	//自动重载开启,手动更新,停止定时器0
	TCON 	= 0x9;			//start timer
	init_irq();	
}

static void init_irq(void)
{
	INTMSK &= ~(1 << 10);	//enable timer0 handle
}
